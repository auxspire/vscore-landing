...
router.get("/match-probability", (req, res) => {
  ...
  if (!teamA) { res.status(400).json({ error: `Team not found: ${teamAId}` }); return; }
  if (!teamB) { res.status(400).json({ error: `Team not found: ${teamBId}` }); return; }

  const numSims = Math.min(20000, Math.max(1000, simulations ? parseInt(simulations as string, 10) : 10000));
...
router.get("/popular-matchups", (_req, res) => {
  ...
  const matchups = POPULAR_PAIRS.map(({ aId, bId, label }) => {
    ...
    return { teamA, teamB, totalProbability: result.totalProbability, mostLikelyStage: bestStage.stage, label };
  }).filter(Boolean);
...
router.get("/bracket-explorer/:teamId", (req, res) => {
  ...
  if (!team) { res.status(404).json({ error: `Team not found: ${req.params.teamId}` }); return; }

  const numSims = Math.min(10000, Math.max(1000, req.query.simulations ? parseInt(req.query.simulations as string, 10) : 5000));
  ...
  try {
    ...
    const path = KNOCKOUT_STAGES.map((stage) => {
      ...
      return { stage, description: sd.description, reachProbability: reachProb, topOpponents };
    });
    res.json({ team, path, tournamentWinProbability: data.winCount / numSims, simulationsRun: numSims });
  } catch (err) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Simulation failed" });
  }
});
...
router.get("/rankings", (req, res) => {
  const numSims = Math.min(20000, Math.max(1000, req.query.simulations ? parseInt(req.query.simulations as string, 10) : 10000));
  try {
...
router.get("/stage-breakdown/:teamId", (req, res) => {
  ...
  try {
    ...
    const stages = Object.entries(reachProbs)
      .map(([stage, probability]) => ({ stage, probability, description: STAGE_DESCRIPTIONS[stage] || stage }))
