...
function simulateGroup(teams: Team[]): GroupResult[] {
  ...
  for (let i = 0; i < teams.length; i++) {
    for (let j = i + 1; j < teams.length; j++) {
      ...
      if (outcome === "A") {
        stats[a.id].points += 3; stats[a.id].gd += baseGd; stats[a.id].goalsFor += baseGd; stats[b.id].gd -= baseGd;
      } else if (outcome === "B") {
        stats[b.id].points += 3; stats[b.id].gd += baseGd; stats[b.id].goalsFor += baseGd; stats[a.id].gd -= baseGd;
      } else {
        stats[a.id].points += 1; stats[b.id].points += 1;
        stats[a.id].goalsFor += 1; stats[b.id].goalsFor += 1;
      }
    }
  }
  ...
  return sorted.map((team, i) => ({
    team, points: stats[team.id].points, gd: stats[team.id].gd, goalsFor: stats[team.id].goalsFor, position: i + 1,
  }));
}
