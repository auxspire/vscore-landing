...
app.post("/auth/link-player-profile", async (c) => {
  try {
    const body = await c.req.json();
...
app.post("/auth/check-unlinked-profiles", async (c) => {
  try {
    const body = await c.req.json();
 ...
 */
app.post("/match-events/:matchId", async (c) => {
  try {
    const matchId = c.req.param('matchId');
 ...
 */
app.get("/match-events/:matchId", async (c) => {
  try {
    const matchId = c.req.param('matchId');
 ...
 */
app.get("/match-events/:matchId/since/:timestamp", async (c) => {
  try {
    const matchId = c.req.param('matchId');
 ...
 */
app.delete("/match-events/:matchId/:eventId", async (c) => {
  try {
    const matchId = c.req.param('matchId');
