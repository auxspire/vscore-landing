...
export const set = async (key: string, value: unknown): Promise<void> => {
  ...
  const { error } = await supabase.from("kv_store_845a157a").upsert({ key, value });
...
export const del = async (key: string): Promise<void> => {
  const supabase = client();
  const { error } = await supabase.from("kv_store_845a157a").delete().eq("key", key);
  if (error) throw new Error(error.message);
};
...
export const mdel = async (keys: string[]): Promise<void> => {
  const supabase = client();
  const { error } = await supabase.from("kv_store_845a157a").delete().in("key", keys);
  if (error) throw new Error(error.message);
};
